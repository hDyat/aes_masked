...
void print_mask(uint8_t mask[10])
{
	printf("Mask\t\t: ");
	for (int i =0; i<10; i++)
	{
...
void print_state(uint8_t round)
{
	printf("State Round[%d]\t\t\t:", round);
	uint8_t i, j;
	for (i = 0; i < 4; ++i)
	{
		for (j = 0; j < 4; ++j)
...
void print_state_after_remask(uint8_t round)
{
	printf("State After Remask Round[%d]\t:", round);
	uint8_t i, j;
	for (i = 0; i < 4; ++i)
	{
		for (j = 0; j < 4; ++j)
...
void print_state_after_subbytes(uint8_t round)
{
	printf("State After Subbytes Round[%d]\t:", round);
	uint8_t i, j;
	for (i = 0; i < 4; ++i)
	{
		for (j = 0; j < 4; ++j)
...
void print_state_after_shiftrow(uint8_t round)
{
	printf("State After Shiftrow Round[%d]\t:", round);
	uint8_t i, j;
	for (i = 0; i < 4; ++i)
	{
		for (j = 0; j < 4; ++j)
...
void print_state_after_mixcolumn(uint8_t round)
{
	printf("State After Mixcolumn Round[%d]\t:", round);
	uint8_t i, j;
	for (i = 0; i < 4; ++i)
	{
		for (j = 0; j < 4; ++j)
...
void print_state_after_addround(uint8_t round)
{
	printf("State After add Round[%d]\t:", round);
	uint8_t i, j;
	for (i = 0; i < 4; ++i)
	{
		for (j = 0; j < 4; ++j)
...
static void CipherMasked(void)
{
	...
	print_state_after_remask(0);
	
	// Masks change from M1',M2',M3',M4' to M
	AddRoundKeyMasked(0);
	print_state_after_addround(0);
	printf("\n");
	...
	for (round = 1;; round++)
	{
		...
		SubBytesMasked();
		print_state_after_subbytes(round);
		//No impact on mask
		ShiftRows();
		print_state_after_shiftrow(round);
		if (round == Nr)
		{
		  break;
		}
		...
		print_state_after_remask(round);
		// Masks change from M1,M2,M3,M4 to M1',M2',M3',M4'
		MixColumns();
		print_state_after_mixcolumn(round);
...
void displayProcess(void){
	//print unmasked key all-round  
	for (int i=0; i<=Nr; ++i)
	{
		printf("RoundKey[%d]\t\t: ", i);
		for (int j=0; j<KEYLEN; ++j)
		{
			if (RoundKey[i*16+j] > 15)
				printf("0x%x ",RoundKey[i*16+j]);
			else 
				printf("0x0%x ", RoundKey[i*16+j]);
		}
		printf("\n");
...
#ifdef SECURE 
	//print masked key all-round 
	for (int i=0; i<=Nr; ++i)
	{
		printf("RoundKeyMasked[%d]\t: ", i);
		for (int j=0; j<KEYLEN; ++j)
		{
			if (RoundKeyMasked[i*16+j] > 15)
				printf("0x%x ", RoundKeyMasked[i*16+j]);
			else 
				printf("0x0%x ", RoundKeyMasked[i*16+j]);
		}
		printf("\n");
	...
	//print distance of key at round 10
	
	for (int i=0; i<=Nr; ++i)
	{
		printf("Round %d distance\t: ", i);
		for (int j=0; j<KEYLEN; ++j)
		{
			uint8_t k1 = RoundKey[i*KEYLEN+j];
			uint8_t k2 = RoundKeyMasked[i*KEYLEN+j];
			uint8_t delta;
...
int main()
{
	...
	uint8_t input[16] = {0x48, 0x45, 0x4c, 0x4f, 0x42, 0x55, 0x44, 0x59, 0x53, 0x41, 0x59, 0x41, 0x4b, 0x52, 0x45, 0x53};
	Key = key;
	KeyExpansion();
